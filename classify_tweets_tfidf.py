...
corpus = train_nepal_sentences + val_nepal_sentences + test_nepal_sentences

vectorizer = TfidfVectorizer()
X = vectorizer.fit_transform(corpus)


train_nepal_ids = np.array(X[:len(train_nepal_sentences)].todense())
val_nepal_ids   = np.array(X[len(train_nepal_sentences) : len(train_nepal_sentences) + len(val_nepal_sentences)].todense())
test_nepal_ids  = np.array(X[len(train_nepal_sentences) + len(val_nepal_sentences) :  len(train_nepal_sentences) + len(val_nepal_sentences) + len(test_nepal_sentences)].todense())
...
config = {'hidden_dropout_prob':0.3, 'num_labels':3,'model_name':'bert-base-uncased', 'hidden_size':train_nepal_ids.shape[1], 'data_dir':'saved_models/',}
